...
public class EmployeeDirectory {
    ...
    public void getEmployeeByExperienceWithFilter(int year) {
        List<Employee> returnList = employeeList.stream().filter(employee -> employee.getWorkExperience() == year).toList();
    ...
    public void getEmployeeByIdFromList(int id) {
        List<Employee> returnList = this.employeeList.stream().filter(employee -> employee.getId() == id).toList();
        for (Employee e : returnList) {
            System.out.println(e);
        }
    }
    ...
    public void getEmployeeByIdFromHashMap(int id) {
        System.out.println((this.employeeHashMap.get(id) != null) ? this.employeeHashMap.get(id) : "С таким ID сотрудников не найдено");
    }
}
